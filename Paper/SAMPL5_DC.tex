...
As described previously, this bootstrapping technique included variation in the experimental values based on their reported uncertainties\cite{Mobley:2014gu}. % other citations
...
For example, consider the number of predictions within one standard deviation of the expected value, the value on the x-axis represents the normal distribution (0.68) and the value on the y-axis will represent the fraction of predictions that are within one model uncertainty of the experimental value.  
...
Many of these predictions used alchemical molecular dynamics simulations to estimate the solvation free energy in explicit solvent using fixed-charge all-atom force fields, all-atom/course-grained hybrid force fields, and polarizing force fields. 
A variety of quantum mechanics (QM) methods were also used including QM/molecular mechanics with implicit or explicit solvent, QM with non-Boltzmann Bennet free energy calculations, and QM energy calculations with a single optimized molecular geometry. 
Two participants used variations on the reference interaction-site model (RISM). % Maybe I should expand on this. 
One participant uses QM calculations to derive parameters for an empirical model for activity coefficients which are used to estimate distribution coefficients. % Paluch. 
A few submissions used empirically trained methods for calculating solvation free energies. % citation
A particularly successful submission, which will be discussed again below, employed Conductor-Like-Screening Model for Real Solvents (COSMO-RS). 
...
While a null hypothesis has no actual predictive power, the performance in this case is an indicator that all other methods need improvements. 
...
We performed a set of blind reference calculations (submission 39) for the SAMPL5 challenge. 
...
Submission 39 was in the top quarter of submissions by most error metrics (Table \ref{groupStats}) although, there is a slight bias for concentration in the cyclohexane layer, evident by the average error ($1.6 \pm 0.3$). 
...
After the challenge we explored how including protonation and deprotonation % is this a word?
...
Next, $\log D$ was calculated using the pKa and partition coefficient determined in submission 39 using equation \ref{acidic} \ref{basic} for acidic basic solutes respectively. 
...
Due to a slight concentration miscalculation and the limit of box size, this is about 100 times more concentrated water to cyclohexane than is determined experimentally. 
...
\caption{} % Need a caption
\end{table}
There is a significant change in the computed solvation free energy for SAMPL5\_074 into cyclohexane with water. 
...
While DMSO and acetonitrile were at very low concentrations, their presence in either solvent layer may affect how a solute is distributed. 
We created topology and coordinate files with 780 water, 130 cyclohexane, 4 DMSO, and 2 acetonitrile molecules using the Solvation Toolkit. 
...
Both DMSO and acetonitrile spend most of the time near the solvent interface, with very little movement into the bulk of the water or cyclohexane. 
...
An important part of making prediction tools usable is accurately knowing how good a prediction will be. 
