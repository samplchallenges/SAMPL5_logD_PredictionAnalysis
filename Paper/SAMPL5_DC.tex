...
As in the SAMPL4 challenge, evaluation of model uncertainty estimates is closely considered, but remains an area where prediction and analysis need improvement. 
In SAMPL4 the top performing submissions achieved a root-mean-squared error (RMSE) around 1.5 kcal/mol. 
If we anticipate accuracy in $\log D$ predictions to be similar to the hydration free energy predictions in SAMPL4, a root-mean-squared error (RMSE) around 1.5 kcal/mol, the expected error here would be around 1.54 log units. 
Only a few submissions had an RMSE below 2.5 log units in their predicted $\log D$ values. 
However, distribution coefficients introduced complexities, not present in past SAMPL challenges, including tautomer enumeration, that are likely to be important in predicting biomolecular properties of interest to drug discovery.
Considering the insight gained by analyzing distribution coefficient predictions here and the fact that they are relatively straightforward to measure experimentally, makes them a promising source of data for future blind challenges. % I know this sentence needs to be fixed, but I've been staring at it for too long and didn't get it reworded on this write up. 
