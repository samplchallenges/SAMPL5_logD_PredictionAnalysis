...
Also, it has been suggested that polar compounds can pull water with them from the aqueous layer into the organic solvent \cite{Leo:1971wu}. 
To explore the effects of water in cyclohexane we performed two additional simulations for SAMPL5\_074.
In both test simulations there were 150 cyclohexane molecules with 1 solute. 
The first with 7 water in the cyclohexane and the next with only 1 water molecule. 
...
Given the number of large, flexible, and polyfunctional compounds in SAMPL5 more investigation into each of these effects along with improved tautomer enumeration would be required to completely understand the slight bias seen here. 
\comment[CCB]{above, I am trying to acknowledge the slight bias for cyclohexane and highlight possible reasons for it}
...
\comment[CCB]{Updated wording above and emphasized the need for improvement}
...
\comment[CCB]{two new paragraphs on sampling and dynamic range}
...
A common trend in these submissions was that the dynamic range in the predicted distribution coefficients was larger than the experimental results, that is, generally the smallest $\log D$ values were under estimated and the largest were over estimated. 
...
For SAMPL5\_074, the introduction of water to cyclohexane increased the $\log D$ estimate from -3.76 to -1.73; it is possible that accounting for water in cyclohexane would correct for the underestimation at the lower end of the $\log D$ values. 
As discussed above, it is possible that insufficient conformational sampling could account for some of this problem. 
