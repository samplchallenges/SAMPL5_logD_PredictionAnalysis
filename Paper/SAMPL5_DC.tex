...
%%%%%%%%
% CCB: Here is the (mostly short) list of things I think might need to be addressed before submitting, I'll include this on Slack too
% Author contact info - see note above?
% Are there other keywords that should be included with the abstract?
% we refer many times to "in the supplementary information" which is actually online with DASH, check with journal if we can call that SI? (I've tried to mark where we say this so its easy to change later?
% Table numbers in the text are assigned by which section they are in, but the caption label is numbered in order, I've indicated references to tables, just in case it needs to be changed.
% There are a few places we say there is analysis "by batch" in the SI, it's not really by batch, like we never looked at statistics for batch 1 or 2 alone, its batch 0, 0-1, or 0-2. I'm not sure this wording is super important, but I indicated the spots in the tex.
...
when the specifications and input files for the challenge were made available on the D3R website (http://drugdesigndata.org); these are also provided in the supporting information.  % SI?
...
\label{MoleculeTable}
\caption{A complete list of compounds used in the SAMPL5 challenge, sorted by batch. The average unsigned error (AUE), reported in log units as SAMPL5 ID: AUE, was calculated with all submitted predictions for that compound.} 
\end{table}

For the prediction of distribution coefficients in SAMPL5, a total of 53 molecules were considered. 
They were assigned an identifier in the form SAMPL5\_XXX and are pictures in table \ref{MoleculeTable}. % table number? 
...
Separate analysis for the other submission options (batch 0 or batches 0 and 1) is available in the supporting information. %SI?
...
However, these metrics were provided to challenge participants and are available in the supporting information. %SI?
...
Here we will primarily focus on just average unsigned error for molecules, but all other error metrics were provided to participants and are available in supporting information. % SI?
...
The input, results, molecular dynamics parameter and corodinate files, and tables of solvation free energies are available in the supporting information. %SI?
...
These are reported for all submissions (Table \ref{groupStats}), % table ref?
...
Comparison and QQ plots for every submission are available in the supporting information as well as error metric tables by batch rather than just for the full set. %SI?
...
This analysis included only submissions that included data for all molecules; the other submissions were indicated in Table \ref{groupStats} % table ref?
...
We performed all our standard error analyses discussed above (RMSE, AUE, and Ave. Err) on this simple model as a point of comparison (Table \ref{otherPredictions}). % table ref?
% tau and R aren't included because you get an error, I assume because they're all zero
...
$XlogP_{oct}$ would be in the top few submissions by tau and R, but ranked in the middle all other metrics (Table \ref{otherPredictions}). % table ref
...
For each molecule, we organized a data set of predicted distribution coefficients and compared them to the experimental values, calculating the average unsigned error for each (Table \ref{MoleculeTable}). % table reference?
...
\end{table}
For both SAMPL5\_050 and SAMPL5\_083 there were significant changes in their calculated solvation free energies and partition coefficients for the two different tautomers (Table \ref{tautomerChanges}). % table ref?
...
\end{table}
With this amount of water in cyclohexane, it results in a significant change in the computed solvation free energy for SAMPL5\_074 into cyclohexane with water (Table \ref{waterInCyc}). % table ref?
...
It should be noted, that the water molecules spend the entire production phase next to the solute, so in general the local concentration of water near the solute may higher than the bulk concentration in cyclohexane.
