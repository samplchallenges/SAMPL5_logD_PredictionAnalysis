...
It is possible the null model would look worse if the experimental results were more evenly dispersed in the entire dynamic range. 
...
Tautomer enumeration is by no means the only complication with this type of calculation, it is simply what we chose to focus on here. 
Many of these molecules are relatively large and insufficient sampling has been shown to dramatically affect calculated partition coefficients \cite{Bannan:2016lp}.
\comment[CCB]{I'm not convinced this is the best place for this or what else to say about it}
...
A detailed understanding of the implications of this for both calculated and measured distribution coefficients would require further study. 
...
Predicting $\log D$ accurately for this set of molecules was rather difficult. 
...
However, considering the null model and $XlogP_{corr}$ both would have been the top submission by RMSE and AUE there is clear room for improve in every method used by SAMPL5 participants. 
...
\comment[CCB]{David - this is really short, I'm not sure if we should expand on this somehow or if it is sufficient to acknowledge the potential problem and cite Luchko}

Another trend for most submissions was that the dynamic range in the predicted distribution coefficients was larger than the experimental results, that is, generally the smallest $\log D$ values were under estimated and the largest were over estimated. 
This dynamic range is visible in figures \ref{examplePlots}, \ref{BestGroups}, and \ref{MobleyPlots}, but was evident in comparison plots for almost all submissions.
For SAMPL5\_074, the introduction of water to cyclohexane increased the $\log D$ estimate from -3.76 to -1.73; it is possible that accounting for this would correct for the underestimation at the lower end of the $\log D$ values. 
Another possibility is a detection limit in the experimental results, if the concentrations in one of the phases was too larger to be accurately measured. 
\comment[CCB]{read this paragraph again and update wording}
