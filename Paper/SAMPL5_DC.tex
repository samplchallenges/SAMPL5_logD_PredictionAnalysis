...
\begin{abstract}
range -3.9 to 2.5 log units (6.4 range)
...
In the recent SAMPL5 challenge, participants submitted predictions for cyclohexane/water distribution coefficients. 
...
The Drug Design Data Resource (D3R), organizers for SAMPL5, settled on cyclohexane-water distribution coefficients, and thanks to a partnership with Genentech, this led to a series of measurements on drug-like compounds, discussed in detail in this issue. %ref Bas' paper
...
Results are reported as the logarithm of this ratio ($\log D$). % probably should cite something here or on the sentence above
These are more complicated than partition coefficients ($\log P$), which are for only a single molecular species of the compound \cite{Leo:1971wu}.
$\log P$ is proportional to the transfer free energy, and can be calculated from solvation free energies. % cite all the computational logP papers. 
In contrast, all tautomers will need to be included to accurately calculate $\log D$, which can be estimated from a calculated $\log P$ and the relative populations of tautomers in each solvent. 
% I added that you can estimate logD from logP so it's clear why we brought up partition coefficients. 
...
We also include details for sets of reference calculations we performed that include 
...
The challenge deadline was February 2, 2016  % found e-mail, just a 24 hour extension
...
QQ Plots compare the fraction of a normal distribution within a number of standard deviations to the distribution of errors (calculated minus experiment) that are within that number of model uncertainties. % CCB: I think this is an improvement
...
As demonstrated in the literature, % cite one of the logP papers referred to above. 
...
A common way to convert between experimentally measured distribution coefficients and partition coefficients is with pKa values for the solute. % cite something
