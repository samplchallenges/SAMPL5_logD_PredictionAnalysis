...
\comment[CCB]{new methods section}
No two solvents are perfectly immiscible. 
We considered the experimental concentration of cyclohexane in water (...) to be too low to affect the calculated distribution coefficients. 
However, water in cyclohexane, with an experimental concentration of ... , has a possibility of affecting the computed solvation free energies, especially for solutes with many polar functional groups. 
Additionally, it has been suggested that more polar molecules may pull water molecules into the organic phase. 
\comment[CCB]{add citations and numbers}
To explore the effects of water in cyclohexane we repeated simulations for SAMPL5\_074 with 1 and 7 water molecules in a box with 150 cyclohexane molecules and 1 solute molecule. 
The input files for this simulation were also created with Solvation Toolkit as described above and the same simulation protocol was followed.

Additionally, in the experimental determination of these distribution coefficients there were small concentrations of dimethyl sulfoxide (DMSO) and acetonitrile. 
The experimental concentrations consisted of each solvent are approximately 50\%, 50\%, 1\%, and 0.4\% by volume for cyclohexane, water, DMSO, and acetonitrile respectively  \cite{Rustenburg:sampl5}.
To explore how the DMSO and acetonitrile distribute between cyclohexane and water we performed a single simulation with 130 cyclohexane, 780 water, 4 DMSO, and 2 acetonitrile molecules. 
...
One participant used Semi-Explicit Assembly, an alchemical solvation free energy method for a rigid solute in an implicit solvent \cite{Brini:sampl5}. 
...
\comment[CCB]{David you had a note about citing existing references, is that something we should be trying to do at this point in the game?}
...
Sebastian Diaz-Rodriguez et. al from Miami University used conservative estimates based on results in previous calculations for solubility and hydration free energy for submissions 53 and 60  \cite{Paluch:sampl5}.
