...
def ComparePlot(x, y, Title, XLabel, YLabel, xerr, yerr, labels, fileName = 'compare.pdf', limits = None, leg = [1.02, 0.98, 2, 1],expError = 1.0, wOption = 1, symbols = ['ro','bs','gD','mx','co','ks'], ax1 = None, white_fill = False): 
    """ Input:
    ...
    for i in range(len(x)):
        if len(x[i]) == 0:
            continue
        if symbols[i][0] == 'k' and white_fill == True:
            p1 = ax1.errorbar(x[i],y[i], xerr = xerr[i], yerr = yerr[i], fmt = symbols[i], mfc = 'white', label = labels[i], capsize = 0.5)
        else:
