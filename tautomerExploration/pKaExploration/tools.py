...
def ComparePlot(x, y, Title, XLabel, YLabel, xerr, yerr, labels, fileName = None, symbols = None):
    """ Input:
    ...
    """
    if symbols == None:
        symbols = ['k.','rs','ms','b^','c^','gs']
    # adjustment so limits are aleast as big as error bars 
    print np.max(xerr)
    adjust = np.max([np.max(xerr), np.max(yerr)]) + 0.2
    ...
    lowLim = np.min([np.min(x), np.min(y)]) - adjust
    highLim = np.max([np.max(y), np.max(x)]) + adjust
    # lowLim = min(min(y)) - adjust
    # highLim = max(max(y)) + adjust
...
def makeQQplot(X, Y, slope, title, xLabel ="Expected fraction within range" , yLabel ="Fraction of predictions within range", fileName = None, uncLabel = 'Model Unc.'):
    """
